...
SETUP INSTRUCTIONS:
	...
	*** RUNNING THE PROGRAM:
		...
		3. Run "ErrorSimulator.java" (Optional, since we are not doingany error simulation at this point in time)
		4. Run "Client.java" (Under "tftp" package)
		5. Enter '1' to send an RRQ OR '2' to send an RRQ.
		6. Type and enter the filename you want to read e.g. enter 'server_big.txt'.
			***NOTE: 
			For RRQ: the file must be in test_files/server folder. (e.g 'server_big.txt')
			For WRQ: the file must be in test_files/client folder. (e.g 'client_big.txt')
		7. The file now should be transferred to the 'test_files/client' folder.
		8. On the server console enter 'exit' to shut down server OR enter 'c' to continue.
		9. On the client console either enter 'exit' to shut down client or repeat step 5 to 8 to continue transfering files.
